...
export const DataGrid = props => {
  const {items, rowClassName = 'admin-grid-row', baseUrl, className, removeAction} = props
  return <div className={`admin-data-grid ${className}`}>
    {items && items.length > 0 ? renderList(items, rowClassName, baseUrl, removeAction)
      : <p className='empty-grid'>Your grid is currently empty</p>}
  </div>
}
